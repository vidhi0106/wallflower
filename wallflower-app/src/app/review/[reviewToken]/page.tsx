  ...
  return (
    <div className="min-h-screen flex justify-center">
      <div className="w-full max-w-sm px-6 py-16" style={{ color: "#4a3d2c" }}>
        <p className="text-[14px] uppercase tracking-wide" style={{ color: "#a8977a" }}>
          {submission.event.recipientName}&rsquo;s {submission.event.occasionText}
        </p>
        <h1 className="font-caveat font-bold text-[32px] mt-1">{submission.contributorName}&rsquo;s bouquet</h1>
        <p className="text-[14px] mt-1" style={{ color: "#a8977a" }}>
          {submission.contributorEmail}
        </p>

        <div className="mt-6 px-4 py-4" style={{ background: "#F7EFDD", borderRadius: 10 }}>
          <p className="text-[16px] whitespace-pre-wrap">{submission.noteText}</p>
          {bouquet && (
            <p className="text-[14px] mt-3" style={{ color: "#7c6a4e" }}>
              {bouquet.color} envelope · {bouquet.flowerIds.length} flower{bouquet.flowerIds.length === 1 ? "" : "s"}
            </p>
          )}
        </div>

        <p className="text-[16px] mt-6 font-nunito font-bold">{STATUS_LABEL[submission.status]}</p>
        ...
        <a
          ...
          className="block text-center text-[14px] mt-6 underline"
          style={{ color: "#a8977a" }}
        >
